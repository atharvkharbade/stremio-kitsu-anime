...
async function enrichKitsuMetadata(metadata, retrieveImdbMetadata) {
  ...
  // 1. Fetch ani.zip mapping metadata (cached dynamically based on airing status)
  const isAiring = metadata.status === 'current' || metadata.status === 'upcoming';
  const ttl = isAiring ? 12 * 60 * 60 : 7 * 24 * 60 * 60; // 12 hours for airing/upcoming, 7 days for finished
  const anizpData = await cacheWrapAnizp(kitsuId, () => fetchAnizpMetadata(kitsuId), { ttl }).catch(() => undefined);
