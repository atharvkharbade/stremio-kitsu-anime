...
const ANIZP_KEY_PREFIX = `${GLOBAL_KEY_PREFIX}|anizp`;
...
const ANIZP_TTL = 7 * 24 * 60 * 60; // 7 days
...
function cacheWrapAnizp(id, method, options) {
  const cacheOptions = options || { ttl: ANIZP_TTL };
  return cacheWrap(`${ANIZP_KEY_PREFIX}:${id}`, method, cacheOptions);
}

module.exports = { cacheWrapCatalog, cacheWrapMeta, cacheWrapImages, cacheWrapIdMapping, cacheWrapAnizp };
